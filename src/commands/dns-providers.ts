...
function showProviderInstructions(providerId: string): void {
  const instructions: Record<string, string[]> = {
    ...
    porkbun: [
      ...
      '     - API Key (pk1_xxx)',
      '     - Secret Key (sk1_xxx)',
      '  4. Both are required for authentication',
    ],
    digitalocean: [
      'How to create a DigitalOcean API Token:',
      '  1. Go to DigitalOcean Control Panel → API (left menu)',
      '  2. Under "Personal access tokens", click "Generate New Token"',
      '  3. Name it (e.g., "mxroute-cli")',
      '  4. Set expiration and select "Full Access" or custom scopes',
      '     (needs Domain: Create, Read, Update, Delete)',
      '  5. Copy the token (shown only once!)',
    ],
    godaddy: [
      'How to get GoDaddy API credentials:',
      '  1. Go to https://developer.godaddy.com and sign in',
      '  2. Navigate to API Keys',
      '  3. Create a new key for the "Production" environment',
      '     (OTE/Test keys only work with api.ote-godaddy.com)',
      "  4. You'll get an API Key and API Secret",
      '',
      '  Note: Production API base: https://api.godaddy.com',
    ],
    hetzner: [
    ...
    vercel: [
      ...
      '     (Make sure you are under Personal Account, not a Team)',
      '  2. Click "Create"',
      '  3. Enter a name and select scope from the dropdown',
